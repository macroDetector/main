...
class VantageUI(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.ver_label = QLabel("ver 0.0.6\nDev by qqqqaqaqaqq")
    ...
    def create_combined_settings_card(self):
        ...
        self.inputs['FEED'] = self.add_grid_input(grid, "FEED_FWD", str(g_vars.dim_feedforward), 3, 3, 
            "FFN 레이어의 내부 차원 크기입니다.")
        self.inputs['DROP'] = self.add_grid_input(grid, "DROP_OUT", str(g_vars.dropout), 4, 0, 
        ...
        self.inputs['WEIGHT'] = self.add_grid_input(grid, "WEIGHT_D", str(g_vars.weight_decay), 6, 3, 
            "L2 정규화 강도입니다. 과적합을 방지합니다.")
        self.inputs['PATIENCE'] = self.add_grid_input(grid, "PATIENCE", str(g_vars.patience), 7, 0, 
            "Early Stopping을 위한 대기 횟수입니다.")
        self.inputs['IMPROVEMENT_CUT'] = self.add_grid_input(grid, "IMPROVEMENT_CUT", str(g_vars.improvement_val_loss_cut), 7, 1, 
            "과적합 방지용 val loss의 하단 리미트 값입니다. 정해진 값은 없습니다. val_loss가 너무 낮다면 (0.001 단위) 값을 올려서 조정해보세요.")

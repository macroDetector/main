...
def points_to_features(df_chunk: pd.DataFrame, seq_len: int = globals.SEQ_LEN, stride: int = globals.STRIDE, log_queue:Queue=None):
    Total_Pass_SEQ = 0
    ...
    def df_to_seq(df: pd.DataFrame):
        ...
        try:
            ...
            for i in range(0, n_rows - seq_len + 1, stride):
                seq = df.iloc[i:i + seq_len][globals.FEATURES].values
            ...
            if len(X) == 0:
                return np.empty((0, seq_len, len(globals.FEATURES)), dtype=np.float32)
